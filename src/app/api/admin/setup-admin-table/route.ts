...
// One-time setup: creates admin_users table and migrates existing super_admin accounts
export async function POST(request: Request) {
  try {
    ...
    // 2. Migrate existing super_admin records from users table -> admin_users
    ...
    return NextResponse.json({
      success: true,
      message: "admin_users table created and migration complete",
      migrated_count: migrated.rows.length,
      total_admins: countRes.rows[0].count,
