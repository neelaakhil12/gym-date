...
export default function GymDetailsPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <button onClick={() => router.push("/explore")} className="text-primary font-bold hover:underline">
            Go back to explore
          </button>
        </div>
      </div>
  ...
  if (paymentLoading) {
    ...
    buttonLabel = "Currently Closed";
  } else if (selectedPlan) {
    buttonLabel = "Pay Rs." + selectedPrice.toLocaleString() + " - Book Now";
  }

  const isButtonDisabled = isClosed || selectedPlanIdx === null || plans.length === 0 || paymentLoading;
          ...
          <div className="lg:relative">
            <div className="bg-white rounded-[32px] p-6 md:p-8 border border-gray-100 shadow-sm lg:sticky lg:top-24 z-10">
                ...
                {isClosed ? (
                  ...
                  plans.map((plan: any, idx: number) => (
                    <label
                      key={plan.id}
                      className={
                        "flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-colors " +
                        (selectedPlanIdx === idx
                      ...
                      <div className="flex items-center">
                        ...
                        <div
                          className={
                            ...
                            (selectedPlanIdx === idx ? "border-primary" : "border-gray-300")
                          }
                        >
              ...
              {paymentStatus && (
                <div
                  className={
                    "flex items-start gap-3 p-4 rounded-2xl mb-4 text-sm font-medium " +
              ...
              <button
                ...
                className={
                  "w-full py-4 rounded-xl text-white font-bold transition-all flex items-center justify-center gap-2 " +
                  (isButtonDisabled
                    ? "bg-gray-300 cursor-not-allowed"
                    : "bg-red-600 hover:bg-red-700 shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95")
                }
              >
              ...
              <div className="flex items-center justify-center gap-2 mt-4">
                ...
                <p className="text-center text-[10px] text-gray-400">
                  {isClosed ? "Bookings will resume once open" : "Secured by Razorpay - UPI, Card, Net Banking & more"}
                </p>
              </div>
            </div>
          </div>
