...
type DashboardTab = 'overview' | 'bookings' | 'virtual_wallet' | 'referral_wallet' | 'scan' | 'edit_gym' | 'withdraw';
...
export const OwnerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const onBackPress = () => {
      ...
      if (activeTab === 'withdraw') {
        setActiveTab(withdrawPayoutType === 'revenue' ? 'virtual_wallet' : 'referral_wallet');
        return true;
      }
      if (activeTab === 'edit_gym') {
  ...
  }, [activeTab, isSidebarOpen, withdrawPayoutType]);
  ...
  const handleWithdrawSubmit = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        setWithdrawSuccessMsg(res.message || 'Your withdrawal request has been submitted to Super Admin!');
  ...
  return (
    <View style={styles.outerContainer}>
      ...
      <View style={[
        ...
        {activeTab === 'withdraw' || activeTab === 'edit_gym' ? (
          <TouchableOpacity 
          ...
          <View>
            ...
            <Text style={{ fontSize: 9.5, fontWeight: '700', color: '#64748B' }}>
              {activeTab === 'overview' ? 'Overview' : 
               activeTab === 'bookings' ? 'Bookings & Revenue' : 
               activeTab === 'virtual_wallet' ? 'Virtual Wallet' : 
               activeTab === 'referral_wallet' ? 'Referral Rewards' : 
               activeTab === 'edit_gym' ? 'Edit Gym Details' : 
               activeTab === 'withdraw' ? (withdrawPayoutType === 'revenue' ? 'Withdraw Revenue Payout' : 'Withdraw Referral Bonus') : 
               'QR Scanner'}
            </Text>
          </View>
      ...
      >
        ...
        {/* ============================================================== */}
        {/* TAB 7: WITHDRAW PAYOUT DEDICATED FULL PAGE                      */}
        {/* ============================================================== */}
        {activeTab === 'withdraw' && (
          <View style={styles.tabContentWrapper}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <TouchableOpacity 
                onPress={() => setActiveTab(withdrawPayoutType === 'revenue' ? 'virtual_wallet' : 'referral_wallet')}
                style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#F1F5F9', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10 }}
                activeOpacity={0.7}
              >
                <ArrowLeft size={16} color="#334155" />
                <Text style={{ fontSize: 13, fontWeight: '700', color: '#334155' }}>
                  Back to {withdrawPayoutType === 'revenue' ? 'Virtual Wallet' : 'Referral Rewards'}
                </Text>
              </TouchableOpacity>
            </View>

            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 20, padding: 18, borderWidth: 1, borderColor: '#E2E8F0', elevation: 2, gap: 14 }}>
              ...
              {withdrawSuccessMsg ? (
                <View style={[styles.successBanner, { paddingVertical: 16 }]}>
                  <CheckCircle2 size={24} color="#059669" style={{ marginRight: 10 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.successBannerText, { fontSize: 14, fontWeight: '800' }]}>{withdrawSuccessMsg}</Text>
                    <TouchableOpacity 
                      onPress={() => setActiveTab(withdrawPayoutType === 'revenue' ? 'virtual_wallet' : 'referral_wallet')}
                      style={{ marginTop: 12, backgroundColor: '#059669', paddingVertical: 8, paddingHorizontal: 14, borderRadius: 8, alignSelf: 'flex-start' }}
                    >
                      <Text style={{ color: '#FFFFFF', fontWeight: '800', fontSize: 12 }}>Return to Wallet</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ) : (
                <>
                  <View style={styles.formFieldGroup}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <Text style={[styles.formLabel, { fontSize: 13, fontWeight: '800', color: '#1E293B' }]}>Withdrawal Amount (₹)</Text>
                      <TouchableOpacity onPress={() => setWithdrawAmount(String(withdrawPayoutType === 'revenue' ? virtualWallet.balance : referralWallet.balance))}>
                        <Text style={{ fontSize: 12, fontWeight: '800', color: THEME.COLORS.primary }}>Set Max</Text>
                      </TouchableOpacity>
                    </View>
                    <TextInput 
                      ...
                      style={[styles.formInput, { height: 48, fontSize: 16, fontWeight: '700', backgroundColor: '#F8FAFC' }]} 
                    />
                  </View>
                  ...
                  {withdrawMethod === 'bank' ? (
                    <View style={{ gap: 12 }}>
                      <View style={styles.formFieldGroup}>
                        <Text style={[styles.formLabel, { fontSize: 12, fontWeight: '700', color: '#475569' }]}>Bank Name</Text>
                        <TextInput value={withdrawBankName} onChangeText={setWithdrawBankName} placeholder="e.g. HDFC Bank, SBI, ICICI" style={styles.formInput} />
                      </View>
                      <View style={styles.formFieldGroup}>
                        <Text style={[styles.formLabel, { fontSize: 12, fontWeight: '700', color: '#475569' }]}>Account Number</Text>
                        <TextInput value={withdrawAccountNumber} onChangeText={setWithdrawAccountNumber} placeholder="0000 0000 0000 0000" keyboardType="numeric" style={styles.formInput} />
                      </View>
                      <View style={styles.formFieldGroup}>
                        <Text style={[styles.formLabel, { fontSize: 12, fontWeight: '700', color: '#475569' }]}>IFSC Code</Text>
                        <TextInput value={withdrawIfsc} onChangeText={setWithdrawIfsc} placeholder="e.g. HDFC0001234" autoCapitalize="characters" style={styles.formInput} />
                      </View>
                    </View>
                  ) : (
                    <View style={{ gap: 14 }}>
                      <View style={styles.formFieldGroup}>
                        <Text style={[styles.formLabel, { fontSize: 12, fontWeight: '700', color: '#475569' }]}>UPI ID / Mobile Number</Text>
                        <TextInput 
                          value={withdrawUpiId} 
                          onChangeText={setWithdrawUpiId} 
                          placeholder="partner@okhdfcbank or 9876543210" 
                      ...
                      <View style={styles.formFieldGroup}>
                        <Text style={[styles.formLabel, { fontSize: 12, fontWeight: '700', color: '#475569' }]}>QR Code / Scanner Screenshot (Optional)</Text>
                        {qrCodePreview ? (
                          ...
                          <TouchableOpacity 
                            ...
                            activeOpacity={0.7}
                          >
                            <Upload size={22} color={THEME.COLORS.primary} style={{ marginBottom: 6 }} />
                  ...
                  <TouchableOpacity 
                    style={[styles.submitWithdrawBtn, { marginTop: 12, paddingVertical: 14, borderRadius: 12 }]}
                  ...
                  >
                    {isWithdrawing ? (
                      ...
                      <Text style={[styles.submitWithdrawBtnText, { fontSize: 15, fontWeight: '900' }]}>Submit Withdrawal Request</Text>
                    )}
                  </TouchableOpacity>
                </>
              )}
